							...
							<tr class="table_row">
								<td class="column-1">
									<div class="how-itemcart1">
										<img src="{{ asset('images/product_images/icons/'.$data['product']['image_icon']) }}" alt="IMG">
									</div>
								</td>
								<td class="column-2">{{ $data->product->firstTranslation()->title }} <br />
									Colour: {{ $data->colour }} <br />
									Size:{{ $data->size }} <br />
								</td>
								<td class="column-3">${{ $data->product->price }} </td>
								<td class="column-4">
									<div class="wrap-num-product flex-w m-l-auto m-r-0">
										...
										<input class="mtext-104 cl3 txt-center num-product" type="number" name="quantity" value="{{ $data['quantity'] }}">
							...
							<div class="p-t-15">
								<form method="post" action="{{ route('front.checkout.store') }}" , enctype='multipart/form-data'>
									...
									<div class="rs1-select2 rs2-select2 bor8 bg0 m-b-12 m-t-9">
										<select name="district" class="js-select2" name="time">
									...
									<div class="bor8 bg0 m-b-22">
										<input name="address" class="stext-111 cl8 plh3 size-111 p-lr-15" type="text" name="postcode" placeholder="Postcode / Zip">
										@error('address')
										<small class="text-danger">{{ $message }}</small>
										@enderror
